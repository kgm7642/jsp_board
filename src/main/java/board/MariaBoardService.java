...
public class MariaBoardService extends ServiceManager implements BoardService {
	...
	// 게시물 전체 개수(검색어o)
	@Override
	public int getTotalCountWithKey(Long boardId, String subject) {
		Connection conn = null;
		PreparedStatement stmt = null;
		int total = 0;
		try {
			conn = getConnection();			
			stmt = conn.prepareStatement("select COUNT(*) as total FROM boardPost WHERE boardId = " + boardId + " AND subject LIKE '%" + subject + "%'");
			ResultSet rs = stmt.executeQuery();
			rs.next();
			total = rs.getInt("total");
	...
	// 댓글 가져오기
	@Override
	public List<BoardComment> getComment(Long boardPostId) {
		Connection conn = null;
		PreparedStatement stmt = null;
		List<BoardComment> comments = new ArrayList<BoardComment>();
		try {
			conn = getConnection();
			stmt = conn.prepareStatement("select a.id, b.id AS userId, a.comment, b.name AS creatorName, a.postId, a.createDate from boardcomment a LEFT OUTER JOIN USER b ON a.userId = b.id WHERE a.postId = " + boardPostId);
			// 쿼리 실행
			ResultSet rs = stmt.executeQuery();
			...
			while(rs.next()){
				...
				boardComment.setId(rs.getLong("id"));
				boardComment.setComment(rs.getString("comment"));
				boardComment.getUser().setName(rs.getString("creatorName"));
				boardComment.getUser().setId(rs.getLong("userId"));
				boardComment.setBoardId(rs.getLong("postId"));
				boardComment.setCreateDate(rs.getTimestamp("createDate"));
	...
	public BoardComment createComment(BoardComment boardComment) {
		...
		try {
			conn = getConnection();
			String query = "INSERT INTO boardcomment (comment, userId, postId, createDate) VALUES (?, ?, ?, NOW())";
			stmt = conn.prepareStatement(query);
			stmt.setString(1, boardComment.getComment());
			stmt.setLong(2, boardComment.getUser().getId());
			stmt.setLong(3, boardComment.getBoardId());
			stmt.executeUpdate();
	...
	public void deleteBoardComment(long id) {
		System.out.println("삭제할 댓글 아이디 : " + id);
		Connection conn = null;
		PreparedStatement stmt = null;
		
		try {
			conn = getConnection();
			String query = "DELETE FROM boardComment WHERE id = " + id;
			stmt = conn.prepareStatement(query);
			stmt.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
